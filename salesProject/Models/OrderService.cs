        ...
        private List<Models.Order> MapOrderDataToList(DataTable orderData, DataTable dtDetails)
        {
            ...
            foreach (DataRow row in orderData.Rows)
            {
                result.Add(new Order()
                {
                    ...
                    Orderdate = row["Orderdate"] == DBNull.Value ? null : String.Format("{0:M/d/yyyy HH:mm:ss}", (DateTime)row["Orderdate"]),
                    OrderId = (int)row["OrderId"],
                    RequireDdate = row["RequireDdate"] == DBNull.Value ? null : String.Format("{0:M/d/yyyy HH:mm:ss}", (DateTime)row["RequireDdate"]),
                    ShipAddress = row["ShipAddress"].ToString(),
                    ShipCity = row["ShipCity"].ToString(),
                    ShipCountry = row["ShipCountry"].ToString(),
                    ShipName = row["ShipName"].ToString(),
                    ShippedDate = row["ShippedDate"] == DBNull.Value ? null : String.Format("{0:M/d/yyyy HH:mm:ss}", (DateTime)row["ShippedDate"]),
